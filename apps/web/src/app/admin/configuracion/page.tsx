...
import { Plus, Server, MapPin, BadgeDollarSign, RefreshCw, Wrench } from 'lucide-react'
  ...
  const submit = async (): Promise<void> => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setKey(''); setName(''); setDuration(''); setShowForm(false)
      } else {
        setFormError(res.error.message)
      }
    } catch {
      setFormError('No se pudo crear el tipo de servicio.')
    } finally {
      setSaving(false)
    }
  }
        ...
        <div className="rounded-lg border border-border bg-bg-elevated p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>Clave</Label>
              <Input value={key} onChange={(e) => setKey(e.target.value)} placeholder="ej: roedor-control" />
            </div>
            <div className="space-y-1">
              <Label>Nombre</Label>
...
function ZonesSection(): JSX.Element {
  ...
  const submit = async (): Promise<void> => {
    ...
    try {
      ...
      setFormError('No se pudo crear la zona.')
    } finally {
      setSaving(false)
    }
  }
...
function PriceListsSection(): JSX.Element {
  ...
  const submit = async (): Promise<void> => {
    ...
    if (!serviceTypeId || !price) { setFormError('Agregá al menos un precio (tipo de servicio + monto).'); return }
    const cents = Math.round(Number(price) * 100)
    if (!Number.isFinite(cents) || cents < 0) { setFormError('El monto no es válido.'); return }
    setSaving(true)
    setFormError(null)
    try {
      ...
      if (res.success) {
        ...
        setName(''); setValidFrom(''); setValidTo(''); setServiceTypeId(''); setPrice(''); setIsDefault(false); setShowForm(false)
      } else {
        setFormError(res.error.message)
      }
    } catch {
      setFormError('No se pudo crear la lista de precios.')
    } finally {
      setSaving(false)
    }
  }
